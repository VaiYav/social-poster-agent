...
|---|---|---:|---:|---|
| `CI-001` Enforce coverage + UI tests in CI | `VERIFY` | P0 | — | `ci.yml` backend+ui jobs pin Node `24.19.0` to the workspace engine contract; clean SHA `462374d` passes unit 207 files/2195 tests, integration 5/39, system 3/46, acceptance 2/82, E2E 7/47; serialized coverage passes 224 files / 2409 tests (2408 passed, 1 skipped, 0 failed) at 75.71/66.54/73.75/77.16 against floors 73/64/70/74. PR #1 attempted the first GitHub Actions run at SHA `bdcc52d`, but both CI and deterministic-evaluation jobs were not started because GitHub reports the account locked due to a billing issue. Original 80/75/80/80 and a green Actions run remain open. Evidence: `packages/backend/artifacts/evaluation/CI-001-local-evidence-2026-08-23.md`; evidence: manual |
| `CI-002` Prod Redis eviction policy fix | `VERIFY` | P0 | — | `docker/docker-compose.prod.yml` shared Redis uses `noeviction` with BullMQ-safety comment + `CHECKPOINT_REDIS_URL` split guidance; maxmemory is 512mb. Local Compose config and local Redis preflight pass; production/staging exact-SHA deployment and live Redis inspection remain. Evidence: `packages/backend/artifacts/evaluation/CI-002-local-evidence-2026-08-23.md`. evidence: manual |
| `REFACTOR-100` Cross-module hygiene fixes | `VERIFY` | P1 | — | `monitoring` injects `ILlmPort` (+ optional-port degradation); infra queue module renamed `QueueInfraModule` (collision removed, all 9 importers updated); `Generate.vue` raw `fetch()` → shared axios instance (UI suite green); both Dockerfiles aligned: dead better-sqlite3 native handling removed (optional peer only), deterministic Camoufox pre-fetch+COPY, heap cap, prisma-schema-before-install, `--filter @spa/backend`, prod `--ignore-scripts`. Current worktree shared/backend build, backend lint/import gate and UI typecheck pass; clean-resource exact-SHA and staging/deployment gates remain. Evidence: `packages/backend/artifacts/evaluation/REFACTOR-100-local-evidence-2026-08-23.md`; evidence: manual |
...
| `EXTRACT-001` Move persona editorial defaults out of code | `HARDEN-001` | `VERIFY` | P2 | `PERSONA-101` | Hardcoded `persona.defaults.ts` removed; runtime now loads validated deployment profiles from `PERSONA_PROFILES_PATH` Markdown/JSON frontmatter, with missing-config empty seeding and CI branding carve-out removed. Focused source/config/build/lint/branding evidence is in `packages/backend/artifacts/evaluation/EXTRACT-001-local-evidence-2026-08-24.md`; clean parent-slice reconciliation and deployment-specific Markdown remain. evidence: manual |
Track rules:
