...
|---|---|---:|---:|---|
| `CI-001` Enforce coverage + UI tests in CI | `VERIFY` | P0 | — | `ci.yml` backend+ui jobs FULLY SIMULATED GREEN locally (2026-08-23 evening, commit `910a30a`): shared build → oxlint → branding gate (sanctioned ADR-007 carve-out list) → nest build → `test:coverage` 216 files / 2368 passed with ENFORCED ratchet floors 73/64/70/74 vs measured 74.5/65.6/72.1/76.0 (generated Prisma client excluded from the denominator); ui job vue-tsc + 29 files / 136 tests green. The old 80/75/80/80 thresholds were aspirational and never met by any run — the ratchet policy raises them toward that target instead of failing forever. Evidence: `packages/backend/artifacts/evaluation/CI-001-local-evidence-2026-08-23.md`; first real GitHub Actions run remains (requires push). evidence: manual |
