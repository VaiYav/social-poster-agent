...
|---|---|---:|---:|---|
| `CI-001` Enforce coverage + UI tests in CI | `VERIFY` | P0 | — | `ci.yml`: backend job switched to `test:coverage` (thresholds 80/75 enforced); `ui` job runs `vue-tsc` + packages/ui Vitest suite. Last complete full layered run is 215 files / 2356 tests / 0 failures at `39304de`; latest focused pillar slice adds 13/13 tests and full rerun is pending parallel-lane quiescence. Latest valid CI-equivalent coverage is 71.62/63.80/70.41/72.94% (statements/branches/functions/lines), still below 80/75/80/80. Evidence: `packages/backend/artifacts/evaluation/CI-001-local-evidence-2026-08-23.md`; first green GitHub Actions run remains. evidence: manual |
