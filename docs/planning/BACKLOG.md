...
|---|---|---:|---:|---|
| `CI-001` Enforce coverage + UI tests in CI | `VERIFY` | P0 | — | `ci.yml`: backend job switched to `test:coverage` (thresholds 80/75 enforced); `ui` job runs `vue-tsc` + packages/ui Vitest suite. Local UI lane is green at 29 files / 136 tests; backend typecheck/lint green. **Full layered suite is green locally as of commit `9ec0825` (2026-08-23 evening): 196 files / 2228 tests across unit+integration+system+e2e+acceptance, 0 failures** — earlier "10 failing" snapshot predates the mock-parity fixes (prisma model stubs for POLICY-102/GROUND/INTEL/CRM, redis mget, Bluesky/Mastodon API-poster overrides, STC-011 stabilization, posts.controller @ApiOperation). Evidence: `packages/backend/artifacts/evaluation/CI-001-local-evidence-2026-08-23.md`; first green GitHub Actions run remains. evidence: manual |
