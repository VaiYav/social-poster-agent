...
| `TGBOT-101` Telegram operator control bot | `CONTROL-001` | `IN_PROGRESS` | P2 | REFACTOR-105 ✅(VERIFY), [Roadmap 16](../roadmap/16-telegram-control-bot.md) | `/status` pipeline snapshot, push alerts (DLQ/bans/failed posts), `/approve` `/reject` drafts, `/pause` `/resume` flow-control mirrors. `chat_id` allowlist; text editing stays in dashboard UI. |
...
| `DOCS-102` VERIFY reconciliation pass | `IN_PROGRESS` | P1 | `DOCS-100`, `DOCS-101` | Classify every `VERIFY` row as `evidence: auto` or `evidence: manual`; archive only rows whose required evidence is fully present, and leave explicit human/external checklists for the rest. |
