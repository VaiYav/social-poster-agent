...
|---|---|---:|---:|---|
| `CI-001` Enforce coverage + UI tests in CI | `VERIFY` | P0 | — | `ci.yml`: backend job switched to `test:coverage` (thresholds 80/75 enforced); new `ui` job runs `vue-tsc` + packages/ui vitest suite (local: 24 files / 113 tests green 2026-08-23). Full-suite coverage run + first green GitHub Actions run remain.  evidence: manual |
| `CI-002` Prod Redis eviction policy fix | `VERIFY` | P0 | — | `docker/docker-compose.prod.yml` shared Redis moved to `noeviction` with BullMQ-safety comment + `CHECKPOINT_REDIS_URL` split guidance; maxmemory raised 256mb→512mb. Compose config parse check pending local docker run.  evidence: manual |
