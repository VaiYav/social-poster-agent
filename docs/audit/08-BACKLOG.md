...
> **FROZEN AUDIT BACKLOG (June 2026 snapshot).** Не брать и не закрывать задачи по
> статусам этого файла. Актуальные task IDs находятся в
> [`docs/planning/BACKLOG.md`](../planning/BACKLOG.md); нерешённые пункты этого аудита
> сначала воспроизводятся через `PLAN-005`.

**Исторический источник порядка исправлений (superseded).** Сводный план: changeset-гигиена из код-ревью (`06`) +
...
`emitDraftGenerated()`. **H2** — найден реальный дубль-вектор и **закрыт** (`bde6c3a`): `withRetry`
в `postById` ре-запускал `postFn` (который **сабмитит**) на сетевых ошибках, а `Timeout`/`net::ERR`
может прилететь **после** сабмита (x.poster кидает при навигации на профиль за пермалинком) → ретрай
постил дубль. Гард `verifyPosted` был только в session-expiry self-recovery, не в сетевом ретрае.
Вынесен общий `findLivePostUrl()` (verifyPosted + isValidPostUrl, fail-safe), `postFn` на ретрае
(только одиночные посты — треды идемпотентны через ThreadProgress) сперва проверяет «уже опубликовано?»
и возвращает существующий URL вместо ре-сабмита; self-recovery переведён на тот же хелпер (+2 теста).
H1 **live-verified** (dry-run): 8 continuation-драфтов, tx чистый, race не воспроизвёлся.
