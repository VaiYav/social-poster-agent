...
async function loadReport() {
  ...
  try {
    ...
    try {
      const summaryRes = await api.get<any>('/analytics/summary');
      report.value = {
        summary: {
  ...
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <Select
          ...
          class="w-48"
          @update:model-value="dateRange = $event as typeof dateRange; loadReport()"
        />
      </div>
    ...
    <div v-else-if="report" class="space-y-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Success Rate" :value="`${report.summary.successRate}%`" :icon="TrendingUp" color="text-status-approved" />
        <StatCard
          label="Avg Quality"
          :value="report.summary.avgQualityScore !== null ? report.summary.avgQualityScore.toFixed(1) : '—'"
          :icon="CheckCircle2"
          color="text-primary"
        />
        <StatCard label="Failed" :value="report.summary.failed" :icon="XCircle" color="text-status-failed" />
      </div>
          ...
          <div class="space-y-4">
            <div class="grid grid-cols-3 gap-3">
              <div class="rounded-lg bg-success/10 p-3 text-center">
                <p class="text-2xl font-bold text-success">{{ report.autoApproveStats.autoApproved }}</p>
                <p class="text-xs text-text-secondary">Auto-Approved</p>
              </div>
              <div class="rounded-lg bg-warning/10 p-3 text-center">
                <p class="text-2xl font-bold text-warning">{{ report.autoApproveStats.humanReview }}</p>
            ...
            <div class="rounded-lg bg-surface-elevated p-3">
              <div class="flex justify-between text-sm">
                ...
                <span class="font-medium text-text-primary">{{ report.autoApproveStats.avgScore.toFixed(1) }} / 10</span>
              </div>
            </div>
            <div v-if="Object.keys(report.byTrigger).length > 0">
              <p class="mb-2 text-xs font-medium text-text-muted uppercase tracking-wide">By Trigger</p>
              <div class="flex flex-wrap gap-2">
                <Badge v-for="(count, trigger) in report.byTrigger" :key="trigger" variant="default">
                  {{ trigger }}: {{ count }}
                </Badge>
              </div>
            </div>
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-center gap-3 text-xs text-text-muted">
              ...
              <Badge v-if="post.qualityScore" variant="default">Q: {{ post.qualityScore }}/10</Badge>
            </div>
