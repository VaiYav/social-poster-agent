...
describe("PostCard component", () => {
  ...
  it("shows network icon and status badge", () => {
    const wrapper = mount(PostCard, {
      props: { post: makePost({ network: "THREADS", status: "POSTED" }) },
    });
  ...
  it("does not show action buttons when showActions=true but status is not DRAFT", () => {
    const wrapper = mount(PostCard, {
      props: { post: makePost({ status: "APPROVED" }), showActions: true },
    });
    expect(wrapper.find("button").exists()).toBe(false);
  });
  ...
  it("truncates content when truncate prop is set", () => {
    ...
    const wrapper = mount(PostCard, {
      props: { post: makePost({ content: longContent }), truncate: 50 },
    });
  ...
  it("shows post URL link when available", () => {
    const wrapper = mount(PostCard, {
      props: { post: makePost({ status: "POSTED", postUrl: "https://x.com/test/status/123" }) },
    });
  ...
  it("shows error message when present", () => {
    const wrapper = mount(PostCard, {
      props: { post: makePost({ status: "FAILED", errorMessage: "Browser timeout" }) },
    });
