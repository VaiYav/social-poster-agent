...
describe('engagement store', () => {
  ...
  it('fetchBrowsingSessions populates sessions', async () => {
    const sessions = [
      { id: 's1', network: 'X', status: 'COMPLETED', postsViewed: 12, interactionsCount: 3, startedAt: '2026-08-07T12:00:00.000Z' },
    ];
  ...
  it('startBrowsingSession posts with network and duration', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/engagement/browsing-session', { network: 'THREADS', durationSec: 600 });
    expect(result).toEqual(session);
  });

  it('fetchAll fetches all endpoints', async () => {
    (api.get as ReturnType<typeof vi.fn>).mockImplementation((url: string) => {
      if (url.startsWith('/engagement/scheduler/status')) return Promise.resolve({ data: { enabled: true } });
  ...
  it('handleSseEvent refetches all on browsing session events', async () => {
    ...
    const store = useEngagementStore();
    const event = { type: 'browsing_session_completed', network: 'X', sessionId: 's1', postsViewed: 5, interactionsCount: 1 } as unknown as SSEvent;
