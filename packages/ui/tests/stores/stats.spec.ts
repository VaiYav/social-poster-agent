...
describe("MOD-06 / stats store", () => {
  ...
  it("UTC-110: fetchStats() populates all 5 dashboard stat card counts", async () => {
    ...
    expect(api.get).toHaveBeenNthCalledWith(2, "/posts", {
      params: { status: "POSTED", limit: 1 },
    });
    expect(api.get).toHaveBeenNthCalledWith(3, "/posts", {
      params: { status: "FAILED", limit: 1 },
    });
    expect(api.get).toHaveBeenNthCalledWith(4, "/posts", {
      params: { status: "APPROVED", limit: 1 },
    });
    expect(api.get).toHaveBeenNthCalledWith(5, "/posts", {
  ...
  it("F3-003: triggerGeneration() sends model override to the backend", async () => {
    (api.post as ReturnType<typeof vi.fn>).mockResolvedValue({
      data: { runId: "r-model", status: "started" },
    });
    (api.get as ReturnType<typeof vi.fn>).mockResolvedValue({ data: [] });
  ...
  it("F3-004: triggerGeneration() omits model when not provided", async () => {
    (api.post as ReturnType<typeof vi.fn>).mockResolvedValue({
      data: { runId: "r-auto", status: "started" },
    });
    (api.get as ReturnType<typeof vi.fn>).mockResolvedValue({ data: [] });
