...
async function buildFullAppModule(): Promise<FullAppResult> {
  ...
  const mockXPoster = {
    post: vi.fn().mockResolvedValue({ url: "https://x.com/exampleco/status/123" }),
  };
...
describe("System Tests: Sessions & Cross-Cutting (STC-026..035, STC-049..052)", () => {
  ...
  it("STC-027: POST /sessions/health-check verifies session validity (REQ-024, HAZ-007)", async () => {
    ...
    const lastHealthCheckUpdate = updateCalls.find((c: unknown[]) => c[0]?.data?.lastHealthCheck);
  ...
  it("STC-028: Auto-login on expired session using env credentials (REQ-025, HAZ-007, HAZ-009)", async () => {
    ...
    const res = await request(app.getHttpServer()).post("/api/v1/posting/post-approved-001");
  ...
  it("STC-029: storageState persisted to Session after posting (REQ-026, HAZ-013)", async () => {
    ...
    const storageStateUpdate = sessionUpdateCalls.find((c: unknown[]) => c[0]?.data?.storageState);
              ...
              if (line.startsWith("data: ")) {
                try {
                  ...
                  if (event.type === "connected") {
                    ...
                    event.postId === "post-reconnect-033"
                  ) {
                    clearTimeout(timeout);
                    res.destroy();
                    resolve(true);
  ...
  it("STC-050: Queue monitoring endpoints return BullMQ job stats (REQ-030, REQ-031)", async () => {
    ...
    const statsRes = await request(app.getHttpServer()).get("/api/v1/queue/X/stats");
    ...
    const failedRes = await request(app.getHttpServer()).get("/api/v1/queue/X/failed");
    ...
    const threadsStatsRes = await request(app.getHttpServer()).get("/api/v1/queue/THREADS/stats");
  ...
  it("STC-051: Scenario 1 — Manual Generation + HITL + Posting (HTTP parts only) (REQ-001,002,005,014,016,018,019,020)", async () => {
    ...
    const approveRes = await request(app.getHttpServer()).post(
      "/api/v1/posts/post-gen-051/approve",
    );
  ...
  it("STC-052: Scenario 4 — Session Expiry + Auto-Login (HTTP parts only) (REQ-024,025,026)", async () => {
    ...
    const postRes = await request(app.getHttpServer()).post("/api/v1/posting/post-approved-001");
