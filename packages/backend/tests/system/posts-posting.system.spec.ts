...
describe("System Tests: Posts & Posting (STC-010..025)", () => {
  ...
  it("STC-010: GET /posts accepts status, network filters and returns paginated results", async () => {
    ...
    const res3 = await request(app.getHttpServer()).get(
      "/api/v1/posts?status=POSTED&network=THREADS",
    );
  ...
  it("STC-016: POST /posts/:id/approve sets APPROVED and records approvedAt; 404 for non-existent", async () => {
    ...
    const res1 = await request(app.getHttpServer()).post("/api/v1/posts/post-draft-x/approve");
    ...
    vi.clearAllMocks();
    prisma.post.findUnique.mockResolvedValue(null);
    const res2 = await request(app.getHttpServer()).post("/api/v1/posts/nonexistent-id/approve");
  ...
  it("STC-017: POST /posts/:id/reject sets REJECTED; 404 for non-existent", async () => {
    ...
    const res1 = await request(app.getHttpServer()).post("/api/v1/posts/post-draft-x/reject");
    ...
    vi.clearAllMocks();
    prisma.post.findUnique.mockResolvedValue(null);
    const res2 = await request(app.getHttpServer()).post("/api/v1/posts/nonexistent-id/reject");
  ...
  it("STC-022: SSE events published on POSTING, POSTED, and FAILED transitions", async () => {
    ...
    const postingEvent2 = publishSpy.mock.calls.find((c: unknown[]) => c[0]?.status === "POSTING");
  ...
  it("STC-025: Idempotency check prevents double-posting (POSTED returns success, POSTING returns error)", async () => {
    ...
    const res1 = await request(app.getHttpServer()).post("/api/v1/posting/post-posted");
    ...
    const res2 = await request(app.getHttpServer()).post("/api/v1/posting/post-posting");
