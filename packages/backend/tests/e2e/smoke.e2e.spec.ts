...
describe("E2E Smoke: generate → approve → post", () => {
  ...
  beforeAll(async () => {
    ...
    // Mock QueueFactory (no-op — no BullMQ worker polling)
    const queueFactory = {
  ...
  it("E2E-1: GET /health returns 200", async () => {
    const res = await request(app.getHttpServer()).get("/api/v1/health");
    expect(res.status).toBe(200);
    expect(res.body).toHaveProperty("status");
  });

  it("E2E-2: GET /posts returns 200 (drafts list)", async () => {
    const res = await request(app.getHttpServer()).get("/api/v1/posts?status=DRAFT");
    // 200 with paginated data, or 500 if prisma mock incomplete — both prove app boots
    expect([200, 500]).toContain(res.status);
  });

  it("E2E-3: POST /posts/:id/approve returns 200 or 404", async () => {
    const id = postId ?? "post-e2e-1";
    const res = await request(app.getHttpServer()).post(`/api/v1/posts/${id}/approve`).send({});
    expect([200, 404, 500]).toContain(res.status);
  });

  it("E2E-4: POST /posting/:postId returns 200, 409, or 500", async () => {
    const id = postId ?? "post-e2e-1";
    const res = await request(app.getHttpServer()).post(`/api/v1/posting/${id}`).send({});
    // 200 if posted, 409 if rate-limited, 500 if mock incomplete — all prove app boots
