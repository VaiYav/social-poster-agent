...
import { CommentStatus, SocialNetwork } from '@prisma/client';
...
import { SHARED_REDIS, SHARED_REDIS_SUBSCRIBER, SHARED_REDIS_PUBLISHER } from '../../src/infrastructure/redis/redis.module.js';
...
function makeMockRedis() {
  ...
  return {
    ...
    set: vi.fn((k: string, v: unknown) => { store.set(k, String(v)); return Promise.resolve('OK'); }),
    setex: vi.fn((k: string, _t: number, v: string) => { store.set(k, v); return Promise.resolve('OK'); }),
  ...
  return {
    ...
    generateChat: vi.fn().mockImplementation((systemPrompt: string) => {
      ...
      if (systemPrompt.includes('Decide whether to reply')) {
        return Promise.resolve({ content: REPLY_DECISION_JSON, model: 'mock', tokens: 20, cost: 0 });
      }
...
describe('F4 RepliesModule integration', () => {
  ...
  it('escalates a toxic comment to human_review at the safety gate', async () => {
    ...
    const fresh = await Test.createTestingModule({
      imports: [ConfigModule.forRoot({ isGlobal: true, ignoreEnvFile: true }), ScheduleModule.forRoot(), NotificationsModule, RepliesModule.withEngagement(TestEngagementModule)],
    })
      .overrideProvider(PrismaService)
      .useValue(mockPrisma)
      .overrideProvider(ILlmPort)
