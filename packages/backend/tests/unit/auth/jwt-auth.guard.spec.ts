...
describe('JwtAuthGuard', () => {
  it('passes through everything when disabled (default)', async () => {
    ...
    expect(await guard.canActivate(ctx({ path: '/api/v1/posts', method: 'GET', headers: {} }, ProtectedController.prototype.handler, ProtectedController))).toBe(true);
  });
  ...
  it('allows access with valid JWT in cookie', async () => {
    ...
    expect(await guard.canActivate(ctx(req, ProtectedController.prototype.handler, ProtectedController))).toBe(true);
  ...
  it('allows access with valid JWT in Authorization: Bearer header', async () => {
    ...
    expect(
      await guard.canActivate(
        ctx({ path: '/api/v1/posts', method: 'GET', headers: { authorization: 'Bearer valid-jwt' } }, ProtectedController.prototype.handler, ProtectedController),
      ),
    ).toBe(true);
  });
  ...
  it('rejects when no token provided', async () => {
    const guard = new JwtAuthGuard(mockJwt(), cfg(ENABLED), new Reflector());
    await expect(
      guard.canActivate(ctx({ path: '/api/v1/posts', method: 'GET', headers: {} }, ProtectedController.prototype.handler, ProtectedController)),
  ...
  it('rejects when token is invalid/expired', async () => {
    ...
    await expect(
      guard.canActivate(ctx({ path: '/api/v1/posts', method: 'GET', headers: { cookie: 'spa_token=bad' } }, ProtectedController.prototype.handler, ProtectedController)),
    ).rejects.toThrow(UnauthorizedException);
  });

  it('leaves /auth/login public even when enabled', async () => {
    const guard = new JwtAuthGuard(mockJwt(), cfg(ENABLED), new Reflector());
    expect(await guard.canActivate(ctx({ path: '/api/v1/auth/login', method: 'POST', headers: {} }, PublicController.prototype.handler, PublicController))).toBe(true);
  });

  it('leaves /health public even when enabled', async () => {
    const guard = new JwtAuthGuard(mockJwt(), cfg(ENABLED), new Reflector());
    expect(await guard.canActivate(ctx({ path: '/api/v1/health', method: 'GET', headers: {} }, PublicController.prototype.handler, PublicController))).toBe(true);
  });

  it('leaves /auth/logout public even when enabled', async () => {
    const guard = new JwtAuthGuard(mockJwt(), cfg(ENABLED), new Reflector());
    expect(await guard.canActivate(ctx({ path: '/api/v1/auth/logout', method: 'POST', headers: {} }, PublicController.prototype.handler, PublicController))).toBe(true);
  });

  it('still gates /health/debug-sentry (not public)', async () => {
    const guard = new JwtAuthGuard(mockJwt(), cfg(ENABLED), new Reflector());
    await expect(
      guard.canActivate(ctx({ path: '/api/v1/health/debug-sentry', method: 'GET', headers: {} }, ProtectedController.prototype.handler, ProtectedController)),
    ).rejects.toThrow(UnauthorizedException);
  });

  it('fails closed when enabled but JWT_SECRET is empty', async () => {
    const guard = new JwtAuthGuard(mockJwt(), cfg({ AUTH_ENABLED: 'true', JWT_SECRET: '' }), new Reflector());
    await expect(
      guard.canActivate(ctx({ path: '/api/v1/posts', method: 'GET', headers: { cookie: 'spa_token=anything' } }, ProtectedController.prototype.handler, ProtectedController)),
    ).rejects.toThrow(UnauthorizedException);
  });
  ...
  it('parses cookie correctly when other cookies are present', async () => {
    ...
    expect(
      await guard.canActivate(
        ...
        }, ProtectedController.prototype.handler, ProtectedController),
      ),
    ).toBe(true);
  });
});
