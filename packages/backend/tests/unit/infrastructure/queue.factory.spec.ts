...
const mocks = vi.hoisted(() => ({
  ...
  queueGetFailed: vi
    .fn()
    .mockResolvedValue([{ id: "failed-job-1", data: { postId: "p1" }, attemptsMade: 3 }]),
...
vi.mock("bullmq", () => ({
  Queue: mocks.QueueMock.mockImplementation(function () {
    return {
      add: mocks.queueAdd,
  ...
  Worker: mocks.WorkerMock.mockImplementation(function () {
    return {
...
describe("QueueFactory (MOD-05 — Infrastructure Adapters)", () => {
  ...
  it("getQueue() lowercases network name in queue name", () => {
    ...
    expect(mocks.QueueMock).toHaveBeenCalledWith("spa-posting-threads", expect.anything());
  });
  ...
  it("getQueue() reuses injected shared Redis connections for client and subscriber", () => {
    ...
    const injectedFactory = new QueueFactory(
      configService,
      discord as never,
      sharedClient,
      sharedSubscriber,
  ...
  it("UTC-463: enqueueEngagement with action=like → correct engagement queue", async () => {
    ...
    expect(mocks.QueueMock).toHaveBeenCalledWith("spa-engagement-x", expect.anything());
    const addArgs = mocks.queueAdd.mock.calls[0]!;
  ...
  it("UTC-464: enqueueEngagement with delay → job.add with delay option", async () => {
    await factory.enqueueEngagement(
      "int-2",
      "THREADS",
      "comment",
      { text: "hi" },
