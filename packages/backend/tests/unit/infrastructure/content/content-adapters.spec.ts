...
describe('ContentAdapterRegistry', () => {
  it('merges and sorts topics from multiple adapters', async () => {
    const adapterA = {
      ...
      fetchTopics: async (): Promise<ContentTopic[]> => [
        { sourceType: 'article', path: 'a-1', topic: 'A article', keywords: [], facts: [], language: 'en' },
      ],
    };
    const adapterB = {
      sourceType: 'b',
      canHandle: () => true,
      fetchTopics: async (): Promise<ContentTopic[]> => [
        { sourceType: 'brief', path: 'b-1', topic: 'B brief', keywords: [], facts: [], language: 'en' },
      ],
    };
  ...
  it('caches results and invalidates on demand', async () => {
    ...
    const registry = new ContentAdapterRegistry(createMockConfigService({ CONTENT_CACHE_TTL_MS: 10000 }), [
  ...
  it('filters readBriefs and readArticles by topic sourceType', async () => {
    const adapter = {
      ...
      canHandle: () => true,
      fetchTopics: async (): Promise<ContentTopic[]> => [
        { sourceType: 'brief', path: 'b-1', topic: 'Brief', keywords: [], facts: [], language: 'en' },
        { sourceType: 'article', path: 'a-1', topic: 'Article', keywords: [], facts: [], language: 'en' },
      ],
    };
...
describe('ApiAdapter', () => {
  ...
  it('maps a root JSON array to ContentTopic', async () => {
    ...
    fetchMock.mockResolvedValue({
      ...
      status: 200,
      json: () => Promise.resolve([
        { title: 'T1', summary: 'S1', keywords: ['k1'], category: 'c1', publishedAt: '2026-01-01T00:00:00Z', link: 'https://x/1' },
      ]),
      text: () => Promise.resolve(''),
    });
  ...
  it('returns empty array on HTTP error and sets lastError', async () => {
    const source: ContentSourceConfig = { sourceType: 'api', config: { url: 'https://api.example.com/topics' } };
    fetchMock.mockResolvedValue({ ok: false, status: 500, text: () => Promise.resolve(''), json: () => Promise.resolve({}) });
    const adapter = new ApiAdapter(source);
    const topics = await adapter.fetchTopics(1);
...
describe('RssAdapter', () => {
  ...
  it('parses RSS <item> into article topics', async () => {
    const source: ContentSourceConfig = { sourceType: 'rss', config: { url: 'https://feed.example.com/rss' } };
  ...
  it('parses Atom <entry> into article topics', async () => {
    const source: ContentSourceConfig = { sourceType: 'rss', config: { url: 'https://feed.example.com/atom' } };
    const xml = `<?xml version="1.0"?>
  ...
  it('returns empty array on HTTP error and sets lastError', async () => {
    const source: ContentSourceConfig = { sourceType: 'rss', config: { url: 'https://feed.example.com/rss' } };
    fetchMock.mockResolvedValue({ ok: false, status: 503, text: () => Promise.resolve(''), json: () => Promise.resolve({}) });
    const adapter = new RssAdapter(source);
    const topics = await adapter.fetchTopics(1);
    expect(topics).toEqual([]);
...
describe('GoogleTrendsAdapter', () => {
  ...
  it('parses Google Trends RSS into topic topics', async () => {
    ...
    const { GoogleTrendsAdapter } = await import('../../../../src/infrastructure/content/adapters/google-trends.adapter');
    ...
    const prisma = {
      contentSource: {
        findMany: vi.fn().mockResolvedValue([
          { sourceType: 'api', name: 'API', enabled: true, priority: 0, config: { url: 'https://api.example.com' } },
          { sourceType: 'rss', name: 'RSS', enabled: true, priority: 1, config: { url: 'https://feed.example.com' } },
        ]),
      },
    };
    const config = createMockConfigService({});
    const adapters = await buildContentAdapters({ configService: config, prisma: prisma as unknown as PrismaService, fsReader, dbReader });
