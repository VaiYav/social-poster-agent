...
describe("F1: EngagementSafetyService", () => {
  ...
  describe("validateUrl", () => {
    it("allows X URLs for X network", () => {
      expect(service.validateUrl(SocialNetwork.X, "https://x.com/user/status/123").allowed).toBe(
        true,
      );
      expect(
        service.validateUrl(SocialNetwork.X, "https://twitter.com/user/status/123").allowed,
      ).toBe(true);
      expect(
        service.validateUrl(SocialNetwork.X, "https://mobile.x.com/user/status/123").allowed,
      ).toBe(true);
    });

    it("allows Threads URLs for Threads network", () => {
      expect(
        service.validateUrl(SocialNetwork.THREADS, "https://threads.net/@user/post/123").allowed,
      ).toBe(true);
      expect(
        service.validateUrl(SocialNetwork.THREADS, "https://www.threads.net/t/123").allowed,
      ).toBe(true);
    });

    it("allows Facebook URLs for Facebook network", () => {
      expect(
        service.validateUrl(SocialNetwork.FACEBOOK, "https://facebook.com/groups/xyz/posts/123")
          .allowed,
      ).toBe(true);
      expect(
        service.validateUrl(SocialNetwork.FACEBOOK, "https://mbasic.facebook.com/story.php?id=123")
          .allowed,
      ).toBe(true);
    });
  ...
  describe("checkContentSafety", () => {
    it("allows safe productivity comment", () => {
      const result = service.checkContentSafety(
        "Product cycle really does feel like a second adolescence.",
      );
