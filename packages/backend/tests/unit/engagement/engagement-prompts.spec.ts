...
describe("Engagement Prompts", () => {
  ...
  it("PR-018c: comment system prompt forbids language mismatch", () => {
    expect(ENGAGEMENT_COMMENT_PROMPT.systemPrompt).toContain(
  ...
  it("PR-024: parses batch with commentText", () => {
    const content =
      '[{"action":"comment","reason":"relevant","confidence":0.8,"commentText":"Great!"}]';
  ...
  it("PR-029: truncates long response to expected count", () => {
    const content =
      '[{"action":"like","reason":"1","confidence":0.9},{"action":"like","reason":"2","confidence":0.9},{"action":"like","reason":"3","confidence":0.9}]';
    const results = parseBatchDecisionResponse(content, 2);
    expect(results).toHaveLength(2);
  });
