...
describe("TargetingService", () => {
  ...
  it("TG-008: home feed URL is correct for each network", () => {
    expect(service.getAvailableSources("X").find((s) => s.source === "home-feed")!.url).toBe(
      "https://x.com/home",
    );
    expect(service.getAvailableSources("THREADS").find((s) => s.source === "home-feed")!.url).toBe(
      "https://www.threads.com/",
    );
  });

  it("TG-009: notifications URL is correct for each network", () => {
    expect(service.getAvailableSources("X").find((s) => s.source === "notifications")!.url).toBe(
      "https://x.com/notifications",
    );
    expect(
      service.getAvailableSources("THREADS").find((s) => s.source === "notifications")!.url,
    ).toBe("https://www.threads.com/activity");
  });

  // ── Config ──

  it("TG-010: uses custom hashtags from config", () => {
    const custom = new TargetingService(
      createMockConfigService({
