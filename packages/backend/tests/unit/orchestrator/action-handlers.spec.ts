...
describe("GeneratePostsHandler", () => {
  function buildHandler() {
    ...
    const autoApproveResults: Map<
      string,
      { decision: "AUTO_APPROVE" | "HUMAN_REVIEW" | "REJECT" | "SKIP" }
    > = new Map();
    const autoApprove = {
      evaluate: vi.fn(
        async (
    ...
    const posts = [
      {
        ...
        llmMetadata: { qualityScore: 8, judgeScores },
      },
      {
        id: "post-2",
        content: "Post 2",
      ...
      },
      {
        ...
        network: SocialNetwork.X,
        status: PostStatus.DRAFT,
        llmMetadata: { qualityScore: "not a number", judgeScores: { anti_ai_tone: "bad" } },
      },
    ];
  ...
  it("passes qualityScore and judgeScores from post.llmMetadata to AutoApproveService.evaluate", async () => {
    ...
    expect(autoApprove.evaluate).toHaveBeenCalledWith(
      "post-1",
      "Post 1",
      SocialNetwork.X,
      8,
      posts[0]!.llmMetadata.judgeScores,
    );
    expect(autoApprove.evaluate).toHaveBeenCalledWith(
      "post-2",
      "Post 2",
