...
describe('buildOrchestratorUserPrompt', () => {
  ...
  it('includes all enabled networks in queue depth and approved drafts', () => {
    const world = makeWorld({
      ...
      drafts: { approved: 6, pending: 0, rejected: 0, approvedByNetwork: { X: 1, THREADS: 2, FACEBOOK: 3 } },
    });
  ...
  it('uses world.now for age calculations, not Date.now', () => {
    ...
    const world = makeWorld({
      now,
      rateLimits: {
        X: { dailyRemaining: 10, weeklyRemaining: 50, dailyLimit: 10, weeklyLimit: 50, minIntervalMs: 0, lastPostMs: now - hourAgo },
        THREADS: { dailyRemaining: 10, weeklyRemaining: 50, dailyLimit: 10, weeklyLimit: 50, minIntervalMs: 0, lastPostMs: 0 },
      },
    });
    const prompt = buildOrchestratorUserPrompt(world);
