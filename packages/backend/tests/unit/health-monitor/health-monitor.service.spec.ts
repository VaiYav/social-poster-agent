...
function postCountByStatus(
  prisma: ReturnType<typeof createMockPrisma>,
  map: Partial<Record<PostStatus, number>>,
) {
  prisma.post.count.mockImplementation(
    (args: { where: { status: PostStatus; approvedAt?: unknown } }) => {
...
describe("HealthMonitorService (F21 — Health Monitor + B3 Reconciliation)", () => {
  ...
  it("B3-010: runReconciliation() proceeds with enqueue when queue.getJob throws (best effort)", async () => {
    ...
    expect(result.requeued).toBe(1);
    expect(ctx.queueService.enqueuePosting).toHaveBeenCalledWith(
      "post-besteffort",
      SocialNetwork.X,
    );
  });
  ...
  it("HC-002: runHealthCheck() emits warning alert when failed posts > 0 (<= 5)", async () => {
    ...
    const failedAlert = report.alerts.find((a) => a.message.includes("FAILED status"));
  ...
  it("HC-006: runHealthCheck() reports post health counts (draft, approved, posting, failed, stuckPosting)", async () => {
    ...
    ctx.prisma.post.count.mockImplementation(
      (args: { where: { status: PostStatus; approvedAt?: unknown } }) => {
  ...
  it("DLQ-003: runHealthCheck() lists active queues with active/waiting jobs", async () => {
    ...
    expect(
      report.queues.activeQueues.some((q) => q.includes("FACEBOOK") && q.includes("5 waiting")),
    ).toBe(true);
  });
  ...
  it("INIT-001: onModuleInit() registers two cron jobs with scheduler registry", () => {
    ...
    expect(ctx.schedulerRegistry.addCronJob).toHaveBeenCalledWith(
      "health-monitor",
      expect.anything(),
    );
    expect(ctx.schedulerRegistry.addCronJob).toHaveBeenCalledWith(
