...
describe("B3: Reconciliation — runReconciliation()", () => {
  ...
  it("B3-REC-025: removes and re-enqueues failed jobs without rate-limit errors", async () => {
    ...
    expect(mockQueueService.enqueuePosting).toHaveBeenCalledWith(
      "post-failed-job",
      SocialNetwork.X,
    );
  });
  ...
  it("B3-REC-014: logs start and completion summary with counts", async () => {
    ...
    // Start log
    expect(loggerSpy).toHaveBeenCalledWith(expect.stringContaining("Running reconciliation"));
    // Completion summary log includes counts
    const summaryCall = loggerSpy.mock.calls.find(
      (c) => typeof c[0] === "string" && c[0].includes("Reconciliation complete"),
    );
  ...
  it("B3-REC-015: logs a warn for each re-enqueued stuck post", async () => {
    ...
    mockPrisma.post.findMany.mockResolvedValue([makeStuckPost("post-warn", SocialNetwork.X, 20)]);
    mockQueue.getJob.mockResolvedValue(null);

    await ctx.service.runReconciliation();

    const stuckWarn = warnSpy.mock.calls.find(
      (c) => typeof c[0] === "string" && c[0].includes("post-warn") && c[0].includes("stuck"),
    );
  ...
  it("B3-REC-016: logs a warn when a post is deduplicated (has active job)", async () => {
    ...
    const dedupWarn = warnSpy.mock.calls.find(
      (c) => typeof c[0] === "string" && c[0].includes("dedup"),
    );
  ...
  it("B3-REC-018: logs an error when enqueue fails and does not publish SSE for that post", async () => {
    ...
    mockPrisma.post.findMany.mockResolvedValue([makeStuckPost("post-err", SocialNetwork.X, 20)]);
    mockQueue.getJob.mockResolvedValue(null);
    ...
    const errCall = errorSpy.mock.calls.find(
      (c) => typeof c[0] === "string" && c[0].includes("post-err"),
    );
  ...
  it("B3-REC-019: proceeds with enqueue when queue getJob throws (best-effort dedup)", async () => {
    ...
    const debugCall = debugSpy.mock.calls.find(
      (c) => typeof c[0] === "string" && c[0].includes("could not check queue state"),
    );
