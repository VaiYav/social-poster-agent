...
const ACCOUNT_X = {
  ...
  handle: "exampleco",
  active: true,
  credentialsRef: "SOCIAL_X_USERNAME,SOCIAL_X_PASSWORD,SOCIAL_X_COOKIES",
};
const ACCOUNT_THREADS = {
  ...
  handle: "exampleco",
  active: true,
  credentialsRef: "SOCIAL_THREADS_USERNAME,SOCIAL_THREADS_PASSWORD,SOCIAL_THREADS_COOKIES",
};
const ACCOUNT_FB = {
  id: "acc-fb",
  ...
  active: true,
  credentialsRef: "SOCIAL_FACEBOOK_USERNAME,SOCIAL_FACEBOOK_PASSWORD,SOCIAL_FACEBOOK_COOKIES",
};
...
function createMockPage(
  opts: { url: string; successVisible: boolean } = {
...
function createMockAccountsService(byNetwork: Record<string, unknown> = {}) {
  ...
  return {
    findByNetwork: vi.fn((network: SocialNetwork) =>
      Promise.resolve(byNetwork[network] ? [byNetwork[network]] : []),
    ),
    findFirstActiveByNetwork: fn,
    findById: vi.fn((id: string) =>
      Promise.resolve(activeAccounts.find((a: any) => a.id === id) ?? null),
    ),
...
describe("MOD-04: SessionsService", () => {
  ...
  async function setup(
    opts: { accounts?: unknown; config?: ConfigService; browser?: unknown; prisma?: unknown } = {},
  ) {
    ...
    if (
      acc &&
      (!acc.getCredentials.getMockImplementation || !acc.getCredentials.getMockImplementation())
    ) {
      acc.getCredentials.mockImplementation((account: any) => {
        const refs = (account.credentialsRef ?? "")
          ...
          .map((s: string) => s.trim())
          .filter(Boolean);
        const result: { username: string; password: string; extra?: string; cookies?: string } = {
          username: "",
          password: "",
        };
  ...
  it("UTC-061: getOrCreateSession() calls autoLogin when no active session exists and returns new session", async () => {
    ...
    expect(t.browser.createContext).toHaveBeenCalledWith(
    ...
    expect(result).toEqual(
      expect.objectContaining({ id: "sess-new-1", status: SessionStatus.ACTIVE }),
    );
  });
  ...
  it("SE1: posting defers (returns null, no form login) when SESSION_DEFERRED_LOGIN=true and only cookies are missing", async () => {
    ...
    expect(t.prisma.session.create).not.toHaveBeenCalled();
    expect(
      (t.discord as unknown as { warning: ReturnType<typeof vi.fn> }).warning,
  ...
  it("UTC-072: healthCheck() updates lastHealthCheck and returns healthy when session valid", async () => {
    ...
    const updateCall = t.prisma.session.update.mock.calls.find(
      (c: unknown[]) =>
        c[0]?.data?.lastHealthCheck instanceof Date && c[0]?.data?.status !== SessionStatus.EXPIRED,
    );
  ...
  it("UTC-080: tryCookieAuth() fails when redirected to login page → returns null, falls back to autoLogin", async () => {
    ...
    browser.createContext.mockResolvedValueOnce(cookieContext).mockResolvedValueOnce(loginContext);
    browser.saveStorageState.mockResolvedValue(JSON.stringify({ cookies: [], origins: [] }));
  ...
  it("UTC-083: circuit breaker opens after 3 failed login attempts → 4th getOrCreateSession returns null", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await expect(
        breaker.execute(() => Promise.reject(new Error("login failed"))),
  ...
  it("UTC-087: decryptStorageState() with encrypted string (v1: prefix) → decrypts and returns JSON string", async () => {
    const t = await setup();
    const enc = t.service["encryptionService"] as {
    ...
    expect(result).toBe(
      JSON.stringify({ cookies: [{ name: "auth", value: "token" }], origins: [] }),
    );
  });
  ...
  it("UTC-091: markSessionExpired() catches prisma errors gracefully (no throw)", async () => {
    ...
    await expect(
      t.service.markSessionExpired(SocialNetwork.X, "sess-456"),
  ...
  it("UTC-092: concurrent getOrCreateSession for same network — second call reuses session from first (sessionLock)", async () => {
    ...
    const ctxPromise = new Promise<unknown>((resolve) => {
      resolveCreateContext = resolve;
    });
  ...
  it("UTC-103: tryCookieAuth() with invalid cookie string (no = signs) → returns null, no createContext", async () => {
    ...
    expect(result).toBeNull();
    expect(t.browser.createContext).not.toHaveBeenCalled();
    const parseWarn = warnSpy.mock.calls.find((c) =>
      /Failed to parse.*cookies/i.test(String(c[0])),
    );
  ...
  it("UTC-104: tryCookieAuth() with missing required cookies → returns null, no createContext", async () => {
    ...
    expect(result).toBeNull();
    expect(t.browser.createContext).not.toHaveBeenCalled();
    const missingWarn = warnSpy.mock.calls.find((c) =>
      /missing required cookies/i.test(String(c[0])),
    );
  ...
  it("UTC-106: autoLogin() FACEBOOK c_user cookie found but session invalid (URL=/login) → proceeds with login form → c_user still present → saves session", async () => {
    ...
    const invalidWarn = warnSpy.mock.calls.find((c) =>
      /c_user cookie found but session invalid/i.test(String(c[0])),
    );
  ...
  it("UTC-108: autoLogin() X — 2FA challenge in headless mode → waits for code via API, returns null on timeout", async () => {
    ...
    const page = createMockPage({ url: "https://x.com/home", successVisible: true });
    const origLocator = page.locator as unknown as (sel: string) => {
      first: () => Record<string, ReturnType<typeof vi.fn>>;
      [k: string]: unknown;
    };
    ...
    const emailReader = t.service["emailReader"] as {
      pollForVerificationCode: ReturnType<typeof vi.fn>;
    };
  ...
  it("UTC-112: concurrent getOrCreateSession — second call waits for lock; SE1 cooldown throttles its back-to-back form login (returns null)", async () => {
    ...
    const ctxPromiseA = new Promise<unknown>((resolve) => {
      resolveCtxA = resolve;
    });
    ...
    browser.createContext.mockReturnValueOnce(ctxPromiseA).mockResolvedValueOnce(contextB);
    browser.saveStorageState.mockResolvedValue(JSON.stringify({ cookies: [], origins: [] }));
  ...
  it("UTC-119: autoLogin() FACEBOOK challenge page in headed mode → c_user cookie detected → session saved", async () => {
    prisma.session.findFirst.mockResolvedValue(null);
    prisma.session.create.mockResolvedValue({
      id: "sess-fb-challenge",
      accountId: ACCOUNT_FB.id,
      status: SessionStatus.ACTIVE,
    });

    const page = createMockPage({
      url: "https://www.facebook.com/challenge",
      successVisible: true,
    });
  ...
  it("UTC-120: autoLogin() FACEBOOK challenge resolved via c_user but post-challenge cookies empty → still on challenge → fail", async () => {
    ...
    expect(result).toBeNull();
    expect(t.prisma.session.create).not.toHaveBeenCalled();
    const failErr = errorSpy.mock.calls.find((c) =>
      /Login failed.*still on.*challenge/i.test(String(c[0])),
    );
    expect(failErr).toBeTruthy();
  });
  ...
  describe("P1-1.1 cleanup: page before context", () => {
    it("tryCookieAuth success closes page and context in order", async () => {
      ...
      const pageCloseLast =
        page.close.mock.invocationCallOrder[page.close.mock.invocationCallOrder.length - 1];
