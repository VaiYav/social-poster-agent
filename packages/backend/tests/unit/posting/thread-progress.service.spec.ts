...
describe('ThreadProgressService (P0-H2 — Resumable Thread Tracking)', () => {
  ...
  it('UTC-TP-008: markReplyFailed handles Prisma error gracefully', async () => {
    ...
    await expect(
      service.markReplyFailed('root-1', 'reply-1', 'error'),
    ).resolves.not.toThrow();
  });
  ...
  it('UTC-TP-011: getThreadProgress returns all entries ordered by position', async () => {
    mockTP.findMany.mockResolvedValue([
      { replyPostId: 'r1', position: 1, status: 'POSTED', postUrl: 'url1', error: null, attemptedAt: new Date(), completedAt: new Date() },
      { replyPostId: 'r2', position: 2, status: 'PENDING', postUrl: null, error: null, attemptedAt: new Date(), completedAt: null },
      { replyPostId: 'r3', position: 3, status: 'FAILED', postUrl: null, error: 'timeout', attemptedAt: new Date(), completedAt: new Date() },
    ]);
