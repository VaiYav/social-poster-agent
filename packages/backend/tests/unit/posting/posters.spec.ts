...
describe("MOD-03: XPoster (BasePoster architecture)", () => {
  ...
  it("BUG-6: postThreadReplies posts every reply and returns a per-reply result", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const results = await (poster as any).postThreadReplies(page, "https://x.com/u/status/1", [
      "r1",
      "r2",
      "r3",
    ]);
  ...
  it("BUG-6: home-page fallback posts the thread replies too (no silent content loss)", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const fbStub = vi
      .spyOn(poster as any, "postViaHomePageCompose")
      .mockResolvedValue({ url: "https://x.com/u/status/root" });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const result = await poster.post(context as unknown, browserPort as unknown, "root tweet", [
      "reply A",
      "reply B",
    ]);
    ...
    expect(replyStub).toHaveBeenCalledWith(page, "https://x.com/u/status/root", [
      "reply A",
      "reply B",
    ]);
...
describe("MOD-03: ThreadsPoster (BasePoster architecture)", () => {
  ...
  it("BUG-6 (Threads): postThreadReplies posts every reply and returns a per-reply result", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const results = await (poster as any).postThreadReplies(
      page,
  ...
  it('BUG-6 (Threads): degraded "cannot validate" path still posts thread replies (no silent loss)', async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const replyStub = vi
      .spyOn(poster as any, "postThreadReplies")
      .mockResolvedValue([{ index: 0, success: true }]);

    const result = await poster.post(context as unknown, browserPort as unknown, "root", [
      "reply A",
    ]);
