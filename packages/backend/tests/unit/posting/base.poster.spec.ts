...
class TestPoster extends BasePoster {
  ...
  async withErrorHandlingPublic(page: any, operation: () => Promise<string | undefined>, context: string) {
  ...
  async retryWithBackoffPublic<T>(operation: () => Promise<T>, maxRetries = 2, baseDelayMs = 5000): Promise<T> {
