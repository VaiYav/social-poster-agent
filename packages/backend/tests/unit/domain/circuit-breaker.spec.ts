...
describe('CircuitBreaker', () => {
  ...
  it('UTC-442: OPEN → timeout → HALF_OPEN (fake timers +60min)', async () => {
    ...
    await expect(breaker.execute(vi.fn().mockRejectedValue(new Error('fail')))).rejects.toThrow('fail');
    expect(breaker.currentState).toBe('OPEN');
  ...
  it('UTC-444: HALF_OPEN trial failure → re-OPEN', async () => {
    ...
    // Act — trial fails
    await expect(breaker.execute(vi.fn().mockRejectedValue(new Error('still failing')))).rejects.toThrow('still failing');
  ...
  describe('CircuitBreakerRegistry', () => {
    ...
    it('UTC-452: Registry.getStates: all breakers states', async () => {
      ...
      // Open X breaker
      await expect(bx.execute(vi.fn().mockRejectedValue(new Error('fail')))).rejects.toThrow('fail');
    ...
    it('UTC-453: Registry.resetAll: all → CLOSED', async () => {
      ...
      await expect(bx.execute(vi.fn().mockRejectedValue(new Error('fail')))).rejects.toThrow('fail');
      await expect(bt.execute(vi.fn().mockRejectedValue(new Error('fail')))).rejects.toThrow('fail');
