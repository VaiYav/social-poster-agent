...
describe("LlmService — quality pass", () => {
  ...
  it("LS-002: different temperatures produce DIFFERENT immutable instances (race fix)", async () => {
    ...
    const groqInstances = mocks.instances.filter(
      (i) => i.ctorArgs.model === "meta-llama/llama-4-scout-17b-16e-instruct",
    );
  ...
  it("LS-002b: maxTokens is part of the instance key (no leakage between calls)", async () => {
    const service = makeService({ GROQ_API_KEY: "key" });
    await service.generateChat("sys", "prompt one", {
      temperature: 0.2,
      maxTokens: 700,
      role: "judge",
    });
  ...
  it("LS-005: LLM_ROLE_CHAINS routes creative roles to the preferred provider first", async () => {
    ...
    const groq = mocks.instances.find(
      (i) => i.ctorArgs.model === "meta-llama/llama-4-scout-17b-16e-instruct",
    );
  ...
  it("LS-008: F3 model override invokes the requested provider/model first and falls back on failure", async () => {
    ...
    await service.generateChat("sys", "prompt one", {
      model: "sambanova/Meta-Llama-3.3-8B-Instruct",
    });
