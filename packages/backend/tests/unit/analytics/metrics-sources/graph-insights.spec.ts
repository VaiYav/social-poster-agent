...
describe('extractMetric (AN1)', () => {
  it('reads a metric from the values[] form (most recent point)', () => {
    expect(extractMetric({ data: [{ name: 'likes', values: [{ value: 1 }, { value: 9 }] }] }, 'likes')).toBe(9);
  });
  it('reads a metric from the total_value form', () => {
    expect(extractMetric({ data: [{ name: 'reposts', total_value: { value: 3 } }] }, 'reposts')).toBe(3);
  });
...
describe('parseGraphInsights (AN1)', () => {
  ...
  it('defaults missing counts to 0 and impressions to null', () => {
    expect(parseGraphInsights({ data: [{ name: 'likes', values: [{ value: 5 }] }] }, THREADS)).toEqual({
      likes: 5,
      comments: 0,
      shares: 0,
      impressions: null,
