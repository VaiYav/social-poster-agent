...
function createMockMetricsRedis() {
  ...
  redis.set.mockImplementation((key: string, _token: string, ...args: unknown[]) => {
  ...
  redis.eval.mockImplementation((script: string, _numKeys: number, key: string, token: string) => {
  ...
  it("F6-009: skips randomDelay for HTTP API sources but keeps it for browser-based sources", async () => {
    ...
    expect(prisma.postMetrics.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ likes: 1, comments: 2, shares: 3, impressions: 4 }),
      }),
    );
