...
if (!KEY) {
  console.error("SESSION_ENCRYPTION_KEY not found");
  process.exit(1);
}
...
async function main() {
  ...
  for (const s of sessions) {
    ...
    try {
      const data = decrypt(storageStr) as {
      ...
      if (auth.length === 0) {
        console.log(
          `# ${network}: no auth cookies found (${cookies.length} total cookies decrypted)`,
        );
        // Debug: list all cookie names + domains
        console.log(
          `#   available cookies: ${cookies.map((c) => `${c.name}(${c.domain})`).join(", ")}\n`,
        );
        continue;
      }
      ...
      console.log(
        `#   storageStr type: ${typeof raw}, starts with: ${storageStr.slice(0, 40)}...\n`,
      );
    }
  }
