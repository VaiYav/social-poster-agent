...
import { PrismaClient, type Post, type Interaction } from '@prisma/client';
...
function isSessionOrAuthFailure(reason = ''): boolean {
  const lower = reason.toLowerCase();
  return /session|login|auth|cookie|banned|suspended|blocked|not found|timeout|connection closed|target page|context or browser/.test(lower);
}
...
function proposeEngagementTriage(interaction: InteractionLike | null | undefined, job: Job, state: string): string {
  if (state === 'unknown') return 'REMOVE_AND_REQUEUE';
  if (state === 'failed') {
    const reason = job.failedReason ?? '';
    if (isRateLimit(reason)) return 'REQUEUE_DELAY';
  ...
  if (interaction.status === 'COMPLETED' || interaction.status === 'FAILED') return 'CLEAR_TERMINAL_JOB';
  return 'REVIEW';
}
...
async function auditEngagementQueue(
  ...
  for (const job of jobs) {
    ...
    let interaction: Pick<Interaction, 'id' | 'status' | 'targetUrl' | 'type' | 'accountId'> | null = null;
    if (interactionId) {
    ...
    rows.push({
      ...
      contentPreview: interaction?.targetUrl ? String(interaction.targetUrl).slice(0, 160) : undefined,
    });
  }
