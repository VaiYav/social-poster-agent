...
function decrypt(s: string): unknown {
  ...
  const d = crypto.createDecipheriv(
    ALGORITHM,
    Buffer.from(envVars.SESSION_ENCRYPTION_KEY, "hex"),
    Buffer.from(iv!, "hex"),
    { authTagLength: 16 },
  );
  d.setAuthTag(Buffer.from(tag!, "hex"));
  return JSON.parse(
    Buffer.concat([d.update(Buffer.from(enc!, "hex")), d.final()]).toString("utf8"),
  );
}
...
async function main() {
  ...
  if (!postId) {
    console.error("Usage: npx tsx scripts/verify-x-post-standalone.ts <post-id>");
    process.exit(1);
  }
  ...
  const storageState = decrypt(storageStr) as {
  ...
  try {
    ...
    const found = await page.evaluate((snippet: string) => {
      ...
      return {
        found: false,
        texts: Array.from(tweets).map((t) => (t.textContent || "").slice(0, 80)),
      };
    }, searchSnippet);
