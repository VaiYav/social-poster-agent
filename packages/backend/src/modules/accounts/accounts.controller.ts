import { Body, Controller, Delete, Get, HttpCode, HttpStatus, Param, ParseEnumPipe, Patch, Query } from '@nestjs/common';
...
export class AccountsController {
  ...
  @ApiQuery({ name: 'network', enum: SocialNetwork, required: false, description: 'Filter by network' })
  @ApiResponse({ status: 200, description: 'List of social accounts (credentials never exposed)' })
  async findAll(@Query('network', new ParseEnumPipe(SocialNetwork, { optional: true })) network?: SocialNetwork) {
  ...
  @Patch(':id')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Update account metadata (credentials are env-driven and cannot be changed here)' })
  @ApiParam({ name: 'id', description: 'Account UUID' })
