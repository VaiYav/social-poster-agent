...
import { PostingModule } from "../posting/posting.module.js";
import { FlowControlModule } from "../flow-control/flow-control.module.js";
import { PostingService } from "../posting/posting.service.js";
...
export class QueueModule implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    if (isDryRun) {
      this.logger.warn(
        "SPA_DRY_RUN=true — BullMQ workers NOT registered (dry-run runner controls flow)",
      );
      return;
    }
        ...
        async (job) => {
          ...
          if (action === "browsing-session") {
            ...
            if (!engagementService) {
              this.logger.warn(`EngagementService unavailable — ${action} job ${job.id} skipped`);
              return;
            }
            switch (action) {
              case "like": {
              ...
              case "repost": {
                if (job.data?.postUrl) {
                  await engagementService.repost(
                    jobNetwork as SocialNetwork,
                    job.data.postUrl as string,
                  );
                }
