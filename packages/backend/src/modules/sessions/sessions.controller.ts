...
export class SessionsController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Run health check on a session (verifies browser session is still valid)",
  })
  @ApiQuery({ name: "network", required: true, enum: ["X", "THREADS", "FACEBOOK"] })
  @ApiQuery({
    name: "accountId",
    required: false,
    description: "Specific account to health-check (defaults to first active for network)",
  })
  ...
  async submitVerifyCode(
    ...
    return {
      success: true,
      message: `Verification code stored for ${network} — login flow will pick it up`,
    };
  }
}
