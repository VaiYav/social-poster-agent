...
const interactionTypeQuerySchema = z
  .enum(Object.values(InteractionType) as [string, ...string[]])
  .optional();
const interactionStatusQuerySchema = z
  .enum(Object.values(InteractionStatus) as [string, ...string[]])
  .optional();
const browsingSessionStatusQuerySchema = z
  .enum(Object.values(BrowsingSessionStatus) as [string, ...string[]])
  .optional();
...
export class EngagementController {
  ...
  async like(@Body() body: unknown) {
    ...
    return this.engagementService.like(parsed.data.network as SocialNetwork, parsed.data.postUrl);
  }
  ...
  async repost(@Body() body: unknown) {
    ...
    return this.engagementService.repost(parsed.data.network as SocialNetwork, parsed.data.postUrl);
  }
