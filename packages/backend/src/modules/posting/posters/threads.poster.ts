...
export class ThreadsPoster extends BasePoster {
  ...
  async post(
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `Compose button click failed: ${(clickErr as Error).message} — trying /compose URL`,
        );
        // Fallback: navigate directly to compose URL
        await page.goto("https://www.threads.com/compose", {
      ...
      if (!profileUrl) {
        ...
        this.logger.warn(
          `Cannot extract profile URL for validation, returning current URL: ${currentUrl}`,
        );
  ...
  private async setComposeText(page: Page, textbox: Locator, content: string): Promise<void> {
    ...
    if (inserted) {
      this.logger.debug(
        "Threads setComposeText via per-character execCommand insertText succeeded",
      );
      return;
    }
  ...
  private async postThreadReplies(
    ...
    for (let i = 0; i < threadItems.length; i++) {
      ...
      try {
        ...
        this.logger.error(
          `Threads reply ${i + 1}/${threadItems.length} failed after retries: ${errMsg}`,
        );
   ...
   */
  private async postReply(page: Page, rootPostUrl: string, content: string): Promise<void> {
    ...
    if (pageText && pageText.includes(contentSnippet)) {
      ...
      this.logger.warn(
        `Threads reply may not have posted — content not found on page after submit`,
      );
    }
  ...
  private async extractProfileUrl(page: Page): Promise<string | null> {
    try {
      ...
      const profileBtn = page
        .locator('a[aria-label*="Profile"], button[aria-label*="Profile"]')
        .first();
