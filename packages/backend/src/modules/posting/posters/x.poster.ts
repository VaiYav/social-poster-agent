...
export class XPoster extends BasePoster {
  ...
  async post(
    ...
    try {
      ...
      if (!enteredText || enteredText.trim().length < 10) {
        ...
        this.logger.debug(
          `X after fill() + nudge — textbox content: "${enteredText.slice(0, 50)}..."`,
        );
      }
      ...
      if (!buttonVisible) {
        ...
        if (fallbackResult) {
          ...
          if (
            ...
            const fbReplyResults = await this.postThreadReplies(
              page,
              fallbackResult.url,
              threadItems,
            );
      ...
      this.logger.log(
        `X post button disabled check: isDisabled=${isDisabled}, aria-disabled=${ariaDisabled}`,
      );
      if (isDisabled) {
        this.logger.warn(
          `X post button is disabled — DraftJS state not updated. Trying fill() to force React state update...`,
        );
        ...
        if (ariaDisabled === "true") isDisabled = true;
        this.logger.log(
          `X after fill() + nudge — button disabled: ${isDisabled}, aria-disabled: ${ariaDisabled}`,
        );
      }
      if (isDisabled) {
        ...
        if (!pasted) {
          ...
          this.logger.warn(
            `X clipboard paste failed — trying humanType (locator.pressSequentially) with slow delay...`,
          );
          await textbox.click({ force: true, timeout: 5000 }).catch(() => {});
          await page.keyboard.press("Control+A").catch(() => {});
          await page.keyboard.press("Backspace").catch(() => {});
        ...
        if (ariaDisabled === "true") isDisabled = true;
        this.logger.log(
          `X after paste/type retry — button disabled: ${isDisabled}, aria-disabled: ${ariaDisabled}`,
        );
      }
      if (isDisabled) {
        ...
        this.logger.warn(
          `X post button still disabled — trying direct beforeinput InputEvent dispatch...`,
        );
        await textbox.click({ force: true, timeout: 5000 }).catch(() => {});
        await page.keyboard.press("Control+A").catch(() => {});
        await page.keyboard.press("Backspace").catch(() => {});
        await this.browser.randomDelay(200, 400);
        const dispatched = await textbox
          .evaluate((el: HTMLElement, value: string) => {
            el.focus();
            try {
        ...
        if (dispatched) {
          ...
          if (ariaDisabled === "true") isDisabled = true;
          this.logger.log(
            `X after beforeinput dispatch — button disabled: ${isDisabled}, aria-disabled: ${ariaDisabled}`,
          );
        }
      }
      if (isDisabled) {
        ...
        this.logger.error(
          `X post button is disabled after all retries — DraftJS state not updated. Aborting.`,
        );
        await this.screenshot(page, "button-disabled-abort");
        return {
          error:
            "Post button is disabled — DraftJS state not updated after all text entry strategies",
          retryable: false,
        };
      }
      ...
      this.logger.log(
        `X textbox content after submit: "${textboxContentAfterSubmit.slice(0, 60)}..." (len=${textboxContentAfterSubmit.length}), stillOnCompose: ${stillOnCompose}`,
      );
      ...
      if (navigatedToHomeWithoutPosting) {
        this.logger.warn(
          `X /compose/post Post button navigated to /home without submitting — trying home page compose dialog...`,
        );
          ...
          if (handle) {
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
              try {
                ...
                this.logger.warn(
                  `X profile validation attempt ${attempt}/3 failed: ${(err as Error).message}`,
                );
                // Page crashed/closed — no point retrying on a dead page
                if (page.isClosed?.()) {
                  this.logger.warn(
                    `X profile validation: page is closed — skipping remaining retries`,
                  );
                  break;
                }
                if (attempt < 3) {
  ...
  private async postViaSideNavCompose(
    ...
    try {
      ...
      let spaMounted = await page
        .waitForSelector('[data-testid="primaryColumn"], [data-testid="SideNav_NewTweet_Button"]', {
          timeout: 30000,
        })
        .then(() => true)
        .catch(() => false);
      ...
      if (!composeVisible) {
        this.logger.warn(
          `X ${label}: compose button not visible after first wait — pausing and retrying...`,
        );
        await this.browser.randomDelay(3000, 5000);
      ...
      this.logger.log(
        `X ${label}: post button disabled check: isDisabled=${fbDisabled}, aria-disabled=${fbAriaDisabled}`,
      );
      if (fbDisabled) {
        ...
        if (fbAriaDisabled === "true") fbDisabled = true;
        this.logger.log(
          `X ${label}: after fill() + nudge — button disabled: ${fbDisabled}, aria-disabled: ${fbAriaDisabled}`,
        );
      }
      if (fbDisabled) {
        ...
        this.logger.warn(
          `X ${label}: post button still disabled — trying direct beforeinput InputEvent dispatch...`,
        );
        await textbox.click({ force: true, timeout: 5000 }).catch(() => {});
        await page.keyboard.press("Control+A").catch(() => {});
        await page.keyboard.press("Backspace").catch(() => {});
        await this.browser.randomDelay(200, 400);
        const fbDispatched = await textbox
          .evaluate((el: HTMLElement, value: string) => {
            el.focus();
            try {
              el.dispatchEvent(
        ...
        if (fbDispatched) {
          ...
          if (fbAriaDisabled === "true") fbDisabled = true;
          this.logger.log(
            `X ${label}: after beforeinput dispatch — button disabled: ${fbDisabled}, aria-disabled: ${fbAriaDisabled}`,
          );
        }
      }
      if (fbDisabled) {
        this.logger.error(
          `X ${label}: post button disabled after all strategies — DraftJS state not updated`,
        );
        await this.screenshot(page, "button-disabled-abort");
        return {
          error: `Post button is disabled — DraftJS state not updated (${label} compose)`,
          retryable: false,
        };
      }
      ...
      this.logger.log(
        `X ${label}: textbox after submit: "${fbTextboxAfterClick.slice(0, 60)}..." (len=${fbTextboxAfterClick.length})`,
      );

      // Fallback to humanClick on Post button if Ctrl+Enter didn't submit and the textbox still has content.
      if (
        !fbClickSuccess ||
        (fbTextboxAfterClick.length > 0 &&
          (page.url().includes("/compose/post") || page.url().includes("/home")))
      ) {
      ...
      this.logger.warn(
        `X ${label}: submitted but no verifiable permalink captured (${currentUrl}) — marking failed`,
      );
