...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    switch (argv[i]) {
      case "--feature":
        args.feature = argv[++i] as Args["feature"];
        break;
      case "--network":
        args.network = argv[++i] as SocialNetwork;
        break;
      case "--count":
        args.count = parseInt(argv[++i] ?? "1", 10);
        break;
      case "--scroll-duration":
        args.scrollDuration = parseInt(argv[++i] ?? "20", 10);
        break;
      case "--yes":
      case "-y":
        args.yes = true;
...
async function runEngagement(app: INestApplication, args: Args): Promise<void> {
  ...
  try {
    ...
    if (postUrls[0]) {
      ...
      console.log(
        "  " + (commentResult.success ? "✓ Commented" : "✗ Comment failed: " + commentResult.error),
      );
    }
    ...
    console.log(
      "  " + (followResult.success ? "✓ Followed" : "✗ Follow failed: " + followResult.error),
    );
    ...
    try {
      if (context) await browser.releaseContext(args.network, context, account.id);
    } catch {
      /* best-effort */
    }
  }
...
async function runTrending(app: INestApplication, args: Args): Promise<void> {
  ...
  google
    .slice(0, 5)
    .forEach((t: ScrapedTrendingTopic, i: number) =>
  ...
  const eventTopics = trendingService
    .getActiveTrending()
    .map((t: TrendingTopic) => ({ topic: t.topic, networks: t.networks }));
